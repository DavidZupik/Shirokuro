...
public class EditStage extends Stage {
    ...
    CellState findNextWhite(int row, int col){
        ...
        for (int i = row+1; i < editState.cells.length; i++) {
            for (int j = 0; j < editState.cells.length; j++) {
                CellState temp = editState.cells[i][j];
    ...
    boolean isSolvable(){
        CellState firstWhite = findNextWhite(0, 0);
        backtracking(firstWhite);
        return pairs.size() == editState.numberOfCircles;
    }


    void backtracking(CellState cell){
        //pozri hore
        if(cell != null) {
            if (cell.getRow() - 1 >= 0) {
                for (int i = cell.getRow() - 1; i >= 0; i--) {
                    if (editState.cells[i][cell.getCol()].state == States.BLACK
                            && editState.cells[i][cell.getCol()].direction == Direction.NONE) {

                        cell.direction = Direction.UP;
                        CellState blackCell = editState.cells[i][cell.getCol()];
                        blackCell.direction = Direction.UP;

                        pairs.put(cell, editState.cells[i][cell.getCol()]);
                        pairs.put(editState.cells[i][cell.getCol()], cell);
                        backtracking(findNextWhite(cell.getRow() - 1, cell.getCol()));
                        break;
                    }
                }
            }
            ...
            if (cell.getCol() + 1 < editState.size) {
                for (int i = cell.getCol() + 1; i < editState.size; i++) {
                    if (editState.cells[cell.getRow()][i].state == States.BLACK
                            && editState.cells[i][cell.getCol()].direction == Direction.NONE) {
                        ...
                        pairs.put(cell, editState.cells[cell.getRow()][i]);
                        pairs.put(editState.cells[cell.getRow()][i], cell);
                        backtracking(findNextWhite(cell.getRow(), cell.getCol() + 1));
                        break;
                    }
                }
            }
            ...
            if (cell.getRow() + 1 < editState.size) {
                for (int i = cell.getRow() + 1; i < editState.size; i++) {
                    if (editState.cells[i][cell.getCol()].state == States.BLACK
                            && editState.cells[i][cell.getCol()].direction == Direction.NONE) {
            ...
            CellState cell1 = pairs.remove(list.get(list.size()-1));
            CellState cell2 = pairs.remove(list.get(list.size()-2));
